package com.haerul.mapples.ui.login;
...
import com.haerul.mapples.BuildConfig;
...
import com.haerul.mapples.databinding.ActivityLoginBinding;
...
public class LoginActivity extends BaseActivity<ActivityLoginBinding, LoginViewModel> implements LoginViewModel.LoginNavigator {
    ...
    private void createFile() {
        ...
        if (!file.exists()) {
            file.mkdir();
            file = new File(Constants.PATH_IMG);
            if (!file.exists()) {
                file.mkdir();
            }
        } else {
        ...
        File file3 = new File(Constants.PATH);
        if (!file3.exists()) {
            file3.mkdir();
            file3 = new File(Constants.PATH_DOWNLOAD);
            if (!file3.exists()) {
                file3.mkdir();
            }
        } else {
        ...
        File file4 = new File(Constants.PATH);
        if (!file4.exists()) {
            file4.mkdir();
            file4 = new File(Constants.PATH_EXPORT_DATA);
            if (!file4.exists()) {
                file4.mkdir();
            }
        } else {
